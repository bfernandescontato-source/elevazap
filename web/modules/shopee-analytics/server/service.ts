  ...
  try {
    do {
      ...
      const items = report.nodes.flatMap(conversion => (conversion.orders || []).flatMap(order => (order.items || []).map((item, index) => ({
        ...
        item_name: item.itemName || "Produto sem nome", shop_id: item.shopId ? String(item.shopId) : null, shop_name: item.shopName || "Loja não informada", item_price: number(item.itemPrice), actual_amount: number(item.actualAmount), refund_amount: number(item.refundAmount), quantity: Math.max(0, Number(item.qty || 0)), commission: number(item.itemTotalCommission), seller_commission: number(item.itemSellerCommission), shopee_commission: number(item.itemShopeeCommissionCapped), seller_commission_rate: item.itemSellerCommissionRate ?? null, shopee_commission_rate: item.itemShopeeCommissionRate ?? null, item_status: item.displayItemStatus || null, complete_time: iso(item.completeTime), image_url: item.imageUrl || null, synced_at: new Date().toISOString()
      }))));
