...
export async function uploadMediaFromStorage(bucket: string, path: string, mimeType: string, fileName: string, connectionId?: string | null): Promise<string> {
  ...
  const response = await graphRequest(`/${phoneNumberId}/media`, { method: "POST", body: form }, connectionId);
