...
export async function testShopeeCredentials(appId: string, appSecret: string) {
  const query = 'mutation { generateShortLink(input: { originUrl: "https://shopee.com.br/product/52377417/6309028319", subIds: ["disparei"] }) { shortLink } }';
  ...
  if (error) {
    ...
    if (code === 11001) throw new Error("A Shopee rejeitou os parâmetros do teste de conexão.");
...
export type ShopeeGraphQlError = { message?: string; extensions?: { code?: number; message?: string } };

export async function shopeeGraphQl<T>(appId: string, appSecret: string, query: string, variables?: Record<string, unknown>) {
  ...
  const body = await response.json().catch(() => ({})) as { data?: T; errors?: ShopeeGraphQlError[] };
  const error = body.errors?.[0];
  if (error) {
    const code = error.extensions?.code;
    if (code === 10020) throw new Error("SHOPEE_AUTH");
    if (code === 10030 || response.status === 429) throw new Error("SHOPEE_RATE_LIMIT");
    if (code === 11001) throw new Error("SHOPEE_INVALID_QUERY");
    throw new Error("SHOPEE_UNAVAILABLE");
  }
  if (!response.ok || !body.data) throw new Error("SHOPEE_UNAVAILABLE");
  return body.data;
}
