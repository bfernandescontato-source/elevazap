...
export async function GET(request: NextRequest) {
  ...
  if (guard.error) return NextResponse.redirect(publicUrl("/admin/mercado-livre-catalog?error=not_admin", request));
  ...
  if (!code || !validState) {
    return NextResponse.redirect(publicUrl("/admin/mercado-livre-catalog?error=invalid_state", request));
  }

  try {
    await completeMercadoLivreAuthorization(code, guard.session.email);
  } catch {
    return NextResponse.redirect(publicUrl("/admin/mercado-livre-catalog?error=exchange_failed", request));
  }
  return NextResponse.redirect(publicUrl("/admin/mercado-livre-catalog?connected=1", request));
}
