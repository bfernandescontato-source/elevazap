...
export async function POST(request: NextRequest) {
  ...
  const { data } = await sb.from(table).update({ status: "pendente", claim_token: null, resolution_note, resolved_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", id).eq("status", "incerto").select("lote_id").maybeSingle();
