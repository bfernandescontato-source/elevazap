...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as { name?: string; flowId?: string; contacts?: RawContact[]; deliverySpeed?: string; connectionId?: string; scheduledAt?: string } | null;
  ...
  if (body?.scheduledAt) {
    ...
    if (Number.isNaN(parsedDate.getTime())) return NextResponse.json({ error: "Data de agendamento inválida." }, { status: 400 });
    if (parsedDate.getTime() < Date.now() - 60_000) return NextResponse.json({ error: "Agendamento no passado." }, { status: 400 });
  ...
  try {
    const result = await createBroadcastAndStart({
      name, flowId, contacts: classified.validContacts, deliverySpeed, connectionId: body?.connectionId, scheduledAt
    });
    if (!result.scheduled) after(() => triggerBatchProcessing(result.broadcastId));
    return NextResponse.json({ ok: true, broadcastId: result.broadcastId, validCount: classified.validCount, duplicateCount: classified.duplicateCount, invalidCount: classified.invalidCount, skippedForPriorRun: result.skippedForPriorRun, scheduled: result.scheduled });
  } catch (error) {
