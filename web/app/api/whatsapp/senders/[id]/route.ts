...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [keys, creds] = await Promise.all([
    sb.from("whatsapp_auth_keys").delete().eq("session_name", sender.session_name).eq("account_id", context.accountId),
    sb.from("whatsapp_auth_creds").delete().eq("session_name", sender.session_name).eq("account_id", context.accountId)
  ]);
  if (keys.error || creds.error) return failure(keys.error || creds.error, "Falha ao remover a conexão deste número.");

  const { error: deleteError } = await sb.from("whatsapp_senders").delete().eq("id", sender.id).eq("account_id", context.accountId);
