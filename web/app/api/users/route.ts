...
import { guardAdminMutation, requireAdminRole } from "@/lib/security";
...
export async function GET() {
  ...
  const { data, error } = await supabaseAdmin().from("app_users")
    .select("id,email,name,role,status,approved_at,created_at")
...
export async function PATCH(request: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin().from("app_users").update(update).eq("id", parsed.data.id)
