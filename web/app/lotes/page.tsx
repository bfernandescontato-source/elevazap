...
export default function LotesPage() {
  ...
  const rows = useMemo(() => {
    ...
    return batches.filter((batch) => JSON.stringify(batch).toLowerCase().includes(query.toLowerCase())).map((batch) => ({ ...batch, campaign: campaignMap.get(batch.campanha_id) || "—", sender: senderMap.get(batch.whatsapp_sender_id) || batch.whatsapp_session_name || "Número principal", modeloNome: batch.modelos_mensagem?.nome || null }));
