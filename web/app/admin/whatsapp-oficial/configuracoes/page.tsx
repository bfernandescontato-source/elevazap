...
export default function OfficialSettingsPage() {
  ...
  const [elevaPay, setElevaPay] = useState<{ url?: string; headerName?: string; event?: string }>({});
  ...
  useEffect(() => {
    ...
    Promise.all([fetch(`/api/admin/official/status?connectionId=${connectionId}`), fetch(`/api/admin/official/templates?connectionId=${connectionId}`), fetch("/api/admin/official/hubla-webhook-url"), fetch("/api/admin/official/elevapay-webhook-config")]).then(async responses => {
      const [s, t, w, e] = await Promise.all(responses.map(r => r.json()));
      if (!active) return;
      if (!responses[0].ok || !responses[1].ok) throw new Error(s.error || t.error || "Falha ao consultar a Meta.");
      setStatus(s); setTemplates(t.templates || []); setWebhook(w.url || ""); setElevaPay(e);
  ...
  return <AppShell title="Configurações da API" subtitle="Conexão, webhooks de compra e testes técnicos. As automações ficam em sua própria página."><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-2xl border border-line bg-panel p-6"><h2 className="font-semibold">Receber compras da ElevaPay</h2><p className="mt-2 text-sm text-muted">Na ElevaPay, crie uma regra de <strong>Venda aprovada</strong> ({elevaPay.event || "order.paid"}) para esta URL. Em Autenticação, escolha <strong>Header</strong> e cole como valor o token <code>ELEVAPAY_OFFICIAL_WEBHOOK_TOKEN</code> configurado no servidor.</p>{elevaPay.url ? <div className="mt-4 flex items-center gap-3 rounded-xl bg-wash p-3"><code className="min-w-0 flex-1 truncate text-xs">{elevaPay.url}</code><CopyButton value={elevaPay.url} /></div> : <p className="mt-3 text-sm text-muted">Webhook não disponível. Confira a configuração do servidor.</p>}<p className="mt-3 text-xs text-muted">O formato atual e o formato SendFlow são aceitos. Se disponível, inclua <code>orderId</code> ou <code>transactionId</code> para identificar compras repetidas do mesmo cliente.</p><Link href="/admin/whatsapp-oficial/automacoes" className="mt-4 inline-block text-sm underline">Configurar automações por produto</Link></section>
