...
export default function OfficialSettingsPage() {
  ...
  return <AppShell title="Configurações da API" subtitle="Conexão, webhooks de compra e testes técnicos. As automações ficam em sua própria página."><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-2xl border border-line bg-panel p-6"><h2 className="font-semibold">Receber compras da ElevaPay</h2><p className="mt-2 text-sm text-muted">Na ElevaPay, crie uma regra de <strong>Venda aprovada</strong> ({elevaPay.event || "order.paid"}) para esta URL. Em Autenticação, envie o token do servidor no header <code>{elevaPay.headerName || "x-elevapay-token"}</code>.</p>{elevaPay.url ? <div className="mt-4 flex items-center gap-3 rounded-xl bg-wash p-3"><code className="min-w-0 flex-1 truncate text-xs">{elevaPay.url}</code><CopyButton value={elevaPay.url} /></div> : <p className="mt-3 text-sm text-muted">Webhook não disponível. Confira a configuração do servidor.</p>}<p className="mt-3 text-xs text-muted">O formato SendFlow também funciona. Se disponível, inclua <code>orderId</code> ou <code>transactionId</code> para identificar compras repetidas do mesmo cliente.</p><Link href="/admin/whatsapp-oficial/automacoes" className="mt-4 inline-block text-sm underline">Configurar automações por produto</Link></section>
