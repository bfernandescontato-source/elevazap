...
type Data = {
  ...
  metrics: { captured_today: number; converted: number; queued: number; queue_limit: number; sent_today: number; ignored: number; conversion_failed: number };
};
...
function offerStatusLabel(offer: { status?: string; error_code?: string }) { return offer.error_code === "PILOT_QUEUE_FULL" ? "Fila cheia" : statusLabel[offer.status || ""] || offer.status || "—"; }
...
export default function AutopilotPage() {
  ...
  async function action(id: string, actionName: "ignore" | "dispatch_now" | "edit" | "retry_conversion" | "retry_queue", text?: string) {
    ...
    setDetail(null); setNotice(actionName === "ignore" ? "Oferta ignorada." : actionName === "edit" ? "Oferta editada." : actionName === "retry_conversion" || actionName === "retry_queue" ? "Oferta adicionada à fila." : "Oferta priorizada para envio."); await load();
  }
  ...
  const visibleOffers = data?.offers.filter((offer) => ["scheduled", "sent"].includes(offer.status)) || [];
  const scheduledOffers = visibleOffers.filter((offer) => offer.status === "scheduled");
  ...
  return <AppShell title="Piloto Automático" subtitle="Automatize a captura e distribuição de ofertas nos seus grupos" action={<button onClick={save} disabled={saving || !form.whatsapp_sender_id} className="h-10 rounded-lg bg-black px-4 text-sm font-medium text-white disabled:opacity-40">{saving ? "Salvando..." : "Salvar"}</button>}>
    ...
    {!data ? <LoadingState /> : <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><Metric label="Capturadas hoje" value={data.metrics.captured_today} /><Metric label="Enviadas hoje" value={data.metrics.sent_today} /></div>
      {data.metrics.queued >= data.metrics.queue_limit ? <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">A fila está cheia. Novas ofertas não serão adicionadas.</div> : null}
      ...
      <section className="rounded-xl border border-line bg-white"><div className="border-b border-line p-5"><Heading title="Ofertas capturadas" description="Acompanhe somente as ofertas programadas e as que já foram enviadas." />{scheduledOffers.length ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><label className="inline-flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={scheduledOffers.every((offer) => selectedOfferIds.includes(offer.id))} onChange={(event) => setSelectedOfferIds(event.target.checked ? scheduledOffers.map((offer) => offer.id) : [])} />Selecionar todas as programadas</label><button type="button" disabled={!selectedOfferIds.length || cancellingOffers} onClick={() => void cancelSelectedOffers()} className="h-9 rounded-lg border border-red-200 px-3 text-sm font-medium text-red-700 disabled:opacity-40">{cancellingOffers ? "Cancelando..." : `Cancelar selecionadas${selectedOfferIds.length ? ` (${selectedOfferIds.length})` : ""}`}</button></div> : null}</div>{visibleOffers.length ? <div><div className="hidden border-b border-line bg-wash px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted lg:grid lg:grid-cols-[28px_minmax(0,2fr)_140px_minmax(220px,1fr)_auto] lg:items-center lg:gap-3"><span /><span>Oferta</span><span>Status</span><span>Agendamento / envio</span><span>Ações</span></div><div className="divide-y divide-line">{visibleOffers.map((offer) => <OfferRow key={offer.id} offer={offer} selected={selectedOfferIds.includes(offer.id)} onSelect={toggleOffer} onOpen={() => void openOffer(offer.id)} onAction={action} />)}</div></div> : <div className="p-10 text-center text-sm text-muted">Nenhuma oferta programada ou enviada ainda.</div>}</section>
    </div>}
...
function OfferRow({ offer, selected, onSelect, onOpen, onAction }: { offer: Offer; selected: boolean; onSelect: (id: string, checked: boolean) => void; onOpen: () => void; onAction: (id: string, action: "ignore" | "dispatch_now" | "retry_queue") => void }) { const canCancel = offer.status === "scheduled"; const dateLabel = offer.scheduled_at ? new Date(offer.scheduled_at).toLocaleString("pt-BR") : "—"; return <div className="grid gap-3 p-4 sm:grid-cols-[28px_minmax(0,1fr)_auto] sm:items-center lg:grid-cols-[28px_minmax(0,2fr)_140px_minmax(220px,1fr)_auto]"><input type="checkbox" aria-label="Selecionar oferta" checked={selected} disabled={!canCancel} onChange={(event) => onSelect(offer.id, event.target.checked)} className="self-start sm:self-center" /><div className="min-w-0"><div className="truncate font-medium text-ink">{offer.original_text?.split("\n")[0] || "Oferta com imagem"}</div><div className="mt-1 truncate text-xs text-muted">Fonte: {groupName(offer) || offer.source_group_id || "—"} · {new Date(offer.captured_at).toLocaleString("pt-BR")}</div></div><div className="sm:col-start-2 lg:col-start-auto"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${offer.status === "sent" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{offerStatusLabel(offer)}</span></div><div className="flex items-center text-sm text-muted sm:col-start-2 lg:col-start-auto"><Clock3 className="mr-1.5 shrink-0" size={14} />{dateLabel}</div><div className="flex gap-2 sm:col-start-3 sm:row-start-1 sm:row-span-4 sm:justify-self-end lg:col-start-auto lg:row-auto"><button onClick={onOpen} className="grid h-9 w-9 place-items-center rounded-lg border border-line" title="Ver"><Eye size={16} /></button>{canCancel ? <button onClick={() => onAction(offer.id, "dispatch_now")} className="grid h-9 w-9 place-items-center rounded-lg border border-line" title="Disparar agora"><Send size={16} /></button> : null}{canCancel ? <button onClick={() => onAction(offer.id, "ignore")} className="h-9 rounded-lg border border-line px-3 text-xs">Ignorar</button> : null}</div></div>; }
function OfferModal({ offer, onClose, onAction }: { offer: Record<string, any>; onClose: () => void; onAction: (id: string, action: "ignore" | "dispatch_now" | "edit" | "retry_conversion" | "retry_queue", text?: string) => void }) { const [text, setText] = useState(offer.processed_text || offer.original_text || ""); const deliveries = offer.offer_deliveries || []; return <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={onClose}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between border-b border-line p-5"><div><h2 className="font-semibold">Detalhe da oferta</h2><p className="mt-1 text-sm text-muted">{offerStatusLabel(offer)}</p></div><button onClick={onClose} className="text-sm text-muted">Fechar</button></div><div className="space-y-5 p-5">{offer.error_code === "PILOT_QUEUE_FULL" ? <div className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">Não entrou porque a fila estava cheia.</div> : null}{offer.media_url ? <img src={offer.media_url} alt="Imagem da oferta" className="max-h-72 w-full rounded-lg bg-wash object-contain" /> : <div className="grid h-28 place-items-center rounded-lg bg-wash text-muted"><ImageIcon /></div>}<div><div className="mb-2 text-sm font-medium">Texto original</div><div className="whitespace-pre-wrap rounded-lg bg-wash p-3 text-sm">{offer.original_text || "—"}</div></div><Field label="Texto que será enviado"><textarea value={text} onChange={(event) => setText(event.target.value)} rows={6} className="focus-ring w-full rounded-lg border border-line p-3 text-sm" /></Field><div className="grid gap-4 sm:grid-cols-2"><Info label="Grupo fonte" value={(Array.isArray(offer.grupos) ? offer.grupos[0]?.nome : offer.grupos?.nome) || offer.source_group_id || "—"} /><Info label="Capturada" value={new Date(offer.captured_at).toLocaleString("pt-BR")} /><Info label="Programada" value={offer.scheduled_at ? new Date(offer.scheduled_at).toLocaleString("pt-BR") : "—"} /><Info label="Conversão afiliada" value={offer.affiliate_conversion_status === "converted" ? "Link convertido" : offer.affiliate_conversion_status === "failed" ? `Falhou: ${offer.affiliate_conversion_error || "verifique a integração"}` : offer.affiliate_conversion_status || "Não necessária"} /></div>{offer.original_link ? <div><div className="text-xs text-muted">Link original</div><a href={offer.original_link} target="_blank" rel="noreferrer" className="mt-1 flex items-center gap-2 break-all text-sm text-blue-700">{offer.original_link}<ExternalLink size={14} /></a></div> : null}{offer.affiliate_link ? <div><div className="text-xs text-muted">Seu link</div><a href={offer.affiliate_link} target="_blank" rel="noreferrer" className="mt-1 flex items-center gap-2 break-all text-sm text-emerald-700">{offer.affiliate_link}<ExternalLink size={14} /></a></div> : null}<div><div className="mb-2 text-sm font-medium">Grupos de destino</div><div className="space-y-2">{deliveries.map((delivery: any) => <div key={delivery.id} className="flex justify-between rounded-lg bg-wash px-3 py-2 text-sm"><span>{(Array.isArray(delivery.grupos) ? delivery.grupos[0]?.nome : delivery.grupos?.nome) || delivery.destination_group_id}</span><span className="text-muted">{delivery.status}</span></div>)}</div></div><div className="flex flex-wrap justify-end gap-2 border-t border-line pt-4">{offer.error_code === "PILOT_QUEUE_FULL" ? <button onClick={() => onAction(offer.id, "retry_queue")} className="h-10 rounded-lg bg-black px-4 text-sm text-white">Tentar novamente</button> : offer.affiliate_conversion_status === "failed" ? <button onClick={() => onAction(offer.id, "retry_conversion")} className="h-10 rounded-lg border border-red-200 px-4 text-sm text-red-700">Tentar novamente</button> : null}{offer.error_code !== "PILOT_QUEUE_FULL" ? <button onClick={() => onAction(offer.id, "edit", text)} className="h-10 rounded-lg border border-line px-4 text-sm">Salvar edição</button> : null}{["ready", "scheduled", "send_failed"].includes(offer.status) ? <button onClick={() => onAction(offer.id, "dispatch_now")} className="h-10 rounded-lg bg-black px-4 text-sm text-white">Disparar agora</button> : null}</div></div></div></div>; }
