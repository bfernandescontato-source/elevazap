...
export default function AutopilotPage() {
  ...
  return <AppShell title="Piloto Automático" subtitle="Automatize a captura e distribuição de ofertas nos seus grupos" action={<button onClick={save} disabled={saving || !form.whatsapp_sender_id} className="h-10 rounded-lg bg-black px-4 text-sm font-medium text-white disabled:opacity-40">{saving ? "Salvando..." : "Salvar"}</button>}>
    ...
    {!data ? <LoadingState /> : <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-6"><Metric label="Capturadas hoje" value={data.metrics.captured_today} /><Metric label="Links convertidos" value={data.metrics.converted} /><Metric label="Fila" value={`${Math.min(data.metrics.queued, data.metrics.queue_limit)} de ${data.metrics.queue_limit}`} /><Metric label="Enviadas hoje" value={data.metrics.sent_today} /><Metric label="Falhas de conversão" value={data.metrics.conversion_failed} /><Metric label="Ignoradas" value={data.metrics.ignored} /></div>
...
function OfferRow({ offer, selected, onSelect, onOpen, onAction }: { offer: Offer; selected: boolean; onSelect: (id: string, checked: boolean) => void; onOpen: () => void; onAction: (id: string, action: "ignore" | "dispatch_now" | "retry_queue") => void }) { const canCancel = !["sent", "sending", "ignored", "duplicate"].includes(offer.status); return <div className="grid gap-3 p-4 lg:grid-cols-[28px_minmax(0,2fr)_1fr_1fr_auto] lg:items-center"><input type="checkbox" aria-label="Selecionar oferta" checked={selected} disabled={!canCancel} onChange={(event) => onSelect(offer.id, event.target.checked)} /><div className="min-w-0"><div className="truncate font-medium text-ink">{offer.original_text?.split("\n")[0] || "Oferta com imagem"}</div><div className="mt-1 text-xs text-muted">Fonte: {groupName(offer) || offer.source_group_id || "—"} · {new Date(offer.captured_at).toLocaleString("pt-BR")}</div></div><div><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${offer.status === "sent" ? "bg-emerald-100 text-emerald-800" : offer.status.includes("failed") ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}>{offerStatusLabel(offer)}</span></div><div className="text-sm text-muted">{offer.error_code === "PILOT_QUEUE_FULL" ? "Não entrou na fila" : offer.scheduled_at ? <><Clock3 className="mr-1 inline" size={14} />{new Date(offer.scheduled_at).toLocaleString("pt-BR")}</> : "Sem agendamento"}</div><div className="flex gap-2"><button onClick={onOpen} className="grid h-9 w-9 place-items-center rounded-lg border border-line" title="Ver"><Eye size={16} /></button>{["ready", "scheduled", "send_failed"].includes(offer.status) ? <button onClick={() => onAction(offer.id, "dispatch_now")} className="grid h-9 w-9 place-items-center rounded-lg border border-line" title="Disparar agora"><Send size={16} /></button> : null}{offer.error_code === "PILOT_QUEUE_FULL" ? <button onClick={() => onAction(offer.id, "retry_queue")} className="h-9 rounded-lg border border-line px-3 text-xs">Tentar novamente</button> : canCancel ? <button onClick={() => onAction(offer.id, "ignore")} className="h-9 rounded-lg border border-line px-3 text-xs">Ignorar</button> : null}</div></div>; }
