...
const statusLabel: Record<string, string> = { captured: "Capturada", processing: "Processando", ready: "Pronta", waiting: "Aguardando vaga", scheduled: "Agendada", sending: "Enviando", sent: "Enviada", ignored: "Ignorada", duplicate: "Duplicada", processing_failed: "Falha no processamento", send_failed: "Falha no envio" };
function offerStatusLabel(offer: { status?: string }) { return statusLabel[offer.status || ""] || offer.status || "—"; }
...
export default function AutopilotPage() {
  ...
  async function action(id: string, actionName: "ignore" | "edit" | "retry_conversion", text?: string) {
    ...
    setDetail(null); setNotice(actionName === "ignore" ? "Oferta ignorada." : actionName === "edit" ? "Oferta editada." : actionName === "retry_conversion" ? "Oferta adicionada à fila." : "Oferta priorizada para envio."); await load();
  }
  ...
  return <AppShell title="Piloto Automático" subtitle="Automatize a captura e distribuição de ofertas nos seus grupos" action={<button onClick={save} disabled={saving || !form.whatsapp_sender_id} className="h-10 rounded-lg bg-black px-4 text-sm font-medium text-white disabled:opacity-40">{saving ? "Salvando..." : "Salvar"}</button>}>
    ...
    {!data ? <LoadingState /> : <div className="space-y-6">
      ...
      <section className="rounded-xl border border-line bg-white p-5"><Heading title="Configuração da automação" description="Defina o ritmo e o horário em que as ofertas podem ser enviadas." /><div className="mt-5 grid gap-5 md:grid-cols-3"><Field label="Enviar 1 oferta a cada"><div className="flex items-center gap-2"><input type="number" min={5} max={1440} value={form.interval_minutes} onChange={(event) => setForm({ ...form, interval_minutes: Number(event.target.value) })} className="focus-ring h-11 w-24 rounded-lg border border-line px-3" /><span className="text-sm text-muted">minutos</span></div></Field><Field label="Início"><input type="time" value={form.operating_start} onChange={(event) => setForm({ ...form, operating_start: event.target.value })} className="focus-ring h-11 rounded-lg border border-line px-3" /></Field><Field label="Fim"><input type="time" value={form.operating_end} onChange={(event) => setForm({ ...form, operating_end: event.target.value })} className="focus-ring h-11 rounded-lg border border-line px-3" /></Field></div>
        ...
        <fieldset className="mt-5"><legend className="text-sm font-medium">Imagem da oferta</legend><div className="mt-2 grid gap-2 sm:grid-cols-2"><label className={`cursor-pointer rounded-lg border p-3 text-sm ${form.keep_original_media ? "border-black bg-wash" : "border-line"}`}><input type="radio" className="mr-2 accent-black" checked={form.keep_original_media} onChange={() => setForm({ ...form, keep_original_media: true })} />Usar imagem original</label><label className={`cursor-pointer rounded-lg border p-3 text-sm ${!form.keep_original_media ? "border-black bg-wash" : "border-line"}`}><input type="radio" className="mr-2 accent-black" checked={!form.keep_original_media} onChange={() => setForm({ ...form, keep_original_media: false })} />Usar imagem do produto pelo link</label></div><p className="mt-2 text-xs text-muted">O card é criado com os dados públicos da URL; se não houver preview, a oferta segue somente com texto e link.</p></fieldset>
        {form.shopee_conversion_enabled || form.mercado_livre_conversion_enabled ? <div className="mt-5"><div className="mb-2 text-sm font-medium">Em caso de erro na conversão</div><label className="mr-5 inline-flex min-h-11 items-center text-sm sm:min-h-0"><input type="radio" className="mr-2 accent-black" checked={form.conversion_failure_policy === "pause"} onChange={() => setForm({ ...form, conversion_failure_policy: "pause" })} />Pausar esta oferta</label><label className="inline-flex min-h-11 items-center text-sm sm:min-h-0"><input type="radio" className="mr-2 accent-black" checked={form.conversion_failure_policy === "send_original"} onChange={() => setForm({ ...form, conversion_failure_policy: "send_original" })} />Enviar mesmo assim</label></div> : null}
      </section>

      <section className="rounded-xl border border-line bg-white"><div className="border-b border-line p-5"><Heading title="Ofertas capturadas" description="Acompanhe somente as ofertas programadas e as que já foram enviadas." />{scheduledOffers.length ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><label className="inline-flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={scheduledOffers.every((offer) => selectedOfferIds.includes(offer.id))} onChange={(event) => setSelectedOfferIds(event.target.checked ? scheduledOffers.map((offer) => offer.id) : [])} />Selecionar todas as programadas</label><button type="button" disabled={!selectedOfferIds.length || cancellingOffers} onClick={() => void cancelSelectedOffers()} className="h-9 rounded-lg border border-red-200 px-3 text-sm font-medium text-red-700 disabled:opacity-40">{cancellingOffers ? "Cancelando..." : `Cancelar selecionadas${selectedOfferIds.length ? ` (${selectedOfferIds.length})` : ""}`}</button></div> : null}</div>{visibleOffers.length ? <div><div className="hidden border-b border-line bg-wash px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted lg:grid lg:grid-cols-[28px_minmax(0,2fr)_140px_minmax(220px,1fr)_176px] lg:items-center lg:gap-3"><span /><span>Oferta</span><span>Status</span><span>Agendamento / envio</span><span>Ações</span></div><div className="divide-y divide-line">{visibleOffers.map((offer) => <OfferRow key={offer.id} offer={offer} selected={selectedOfferIds.includes(offer.id)} onSelect={toggleOffer} onOpen={() => void openOffer(offer.id)} onAction={action} />)}</div></div> : <div className="p-10 text-center text-sm text-muted">Nenhuma oferta programada ou enviada ainda.</div>}</section>
    </div>}
