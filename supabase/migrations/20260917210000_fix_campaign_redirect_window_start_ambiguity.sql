-- resolve_campaign_redirect(p_slug, p_utm, p_user_agent, p_anonymous_session_id,
-- p_rate_limit_key) declares local variables window_start/expires_at with the
-- same names as rate_limits.window_start/expires_at. Inside the "insert ...
-- on conflict (key, scope, window_start) do update" statement this is
-- ambiguous, so every call with a rate limit key has been failing at runtime
-- with "column reference \"window_start\" is ambiguous" (visible in
-- postgres logs, several times per minute). The route that calls this RPC
-- (app/c/[slug]/route.ts) silently falls back to a non-atomic path when the
-- RPC errors, so visitors still get redirected but without the atomic
-- capacity check or per-visitor rate limiting.
...
begin
  ...
  if p_rate_limit_key is not null then
    ...
    v_expires_at := v_window_start + interval '2 minutes';
    insert into public.rate_limits(key, scope, count, window_start, expires_at, created_at, updated_at)
    values (left(p_rate_limit_key, 256), 'campaign_link:' || selected_campaign.id::text, 1, v_window_start, v_expires_at, now(), now())
