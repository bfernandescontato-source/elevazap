...
async function main() {
  ...
  setTimeout(() => syncAllCampaignGroups(runtime).catch((e) => console.error("[groups] initial sync error:", e)), 15_000);
  setInterval(() => syncAllCampaignGroups(runtime).catch((e) => console.error("[groups] periodic sync error:", e)), 5 * 60_000);
