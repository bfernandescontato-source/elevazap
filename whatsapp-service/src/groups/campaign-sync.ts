...
export async function syncAllCampaignGroups(runtime: WhatsAppRuntime) {
  ...
  const batches = new Map<string, { senderId: string | null; sessionName: string | null; campaignIds: string[]; groupJids: Set<string> }>();
  for (const campaign of campaigns || []) {
    const sender = Array.isArray(campaign.whatsapp_senders) ? campaign.whatsapp_senders[0] : campaign.whatsapp_senders;
    const key = sender?.session_name || "principal";
    const batch = batches.get(key) || { senderId: campaign.whatsapp_sender_id, sessionName: sender?.session_name || null, campaignIds: [] as string[], groupJids: new Set<string>() };
  ...
  for (const batch of batches.values()) {
    ...
    try {
      ...
      await persist(batch.campaignIds, batch.senderId, rows);
