...
window.addEventListener("message", (event) => {
    if (event.source !== window || event.origin !== window.location.origin || event.data?.type !== CONNECT)
        return;
    const { nonce, backendOrigin } = event.data;
    if (typeof nonce !== "string" || nonce.length < 32 || backendOrigin !== window.location.origin)
        return;
